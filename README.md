...
# need disable Magento_TwoFactorAuth
```

```shell
php -d memory_limit=-1 bin/magento dev:tests:run
```


```shell
  ./vendor/bin/phpunit -vvv  -c dev/tests/unit/phpunit.xml.dist dev/tests/unit
```